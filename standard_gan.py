...
# 创建文件夹
if not os.path.exists('./standardgan_img'):
    os.mkdir('./standardgan_img')
...
for epoch in range(num_epoch):  # 进行多个epoch的训练
    for i, (img, _) in enumerate(dataloader):
        ...
        if epoch == 0:
            ...
            save_image(real_images, './standardgan_img/real_images.png')
    fake_images = to_img(fake_img.cpu().data)
    save_image(fake_images, './standardgan_img/fake_images-{}.png'.format(epoch + 1))
 
# 保存模型
torch.save(G.state_dict(), './standardgan_img/generator.pth')
torch.save(D.state_dict(), './standardgan_img/discriminator.pth')
